...
def _declare_prototypes(cg: ctypes.CDLL, cf: ctypes.CDLL, aps: ctypes.CDLL) -> None:
    ...
    # CGEventCreateScrollWheelEvent is variadic (`..., int32_t wheel1, ...`),
    # which ctypes cannot call safely on Apple Silicon: arm64 passes variadic
    # arguments on the stack while a fixed ctypes prototype passes them in
    # registers, so wheel2 would arrive as garbage. The `2` variant has a
    # fixed six-argument signature and identical semantics.
    ...
    cg.CGEventCreateScrollWheelEvent2.argtypes = [
        ...
        c_int32,  # wheel3 (unused third axis)
    ]
...
class MacOSBackend(InputBackend):
    ...
    def _current_point(self) -> CGPoint:
        ...
        if not event:
            # CFRelease(NULL) and CGEventGetLocation(NULL) both crash the
            # process, so a failed allocation must surface as an exception.
            raise RuntimeError(
                "CGEventCreate returned NULL; cannot read the cursor position. "
                "The window server connection may be unavailable (e.g. an SSH "
                "session outside the login user's GUI context)."
            )
    ...
    def _post_and_release(self, event: Any) -> None:
        """Post an injected event to the HID tap and release it, guarding NULL.

        CGEventPost(tap, NULL) is tolerated by the OS, but CFRelease(NULL)
        crashes the process - so a failed event allocation is skipped whole.
        """
        if not event:
            return
    ...
    def scroll(self, sdx: int, sdy: int) -> None:
        ...
        event = self._cg.CGEventCreateScrollWheelEvent2(
            None, K_CG_SCROLL_EVENT_UNIT_LINE, 2, int(sdy), int(sdx), 0
        )
        self._post_and_release(event)
