...
class _FakeCG:
    ...
    def CGEventCreateScrollWheelEvent2(self, *args: object) -> int:  # noqa: N802
        self.calls.append(("CGEventCreateScrollWheelEvent2", args))
        return 3
...
def test_scroll_passes_vertical_then_horizontal_to_scroll_wheel_event() -> None:
    ...
    Regression guard for a natural mix-up: CGEventCreateScrollWheelEvent2's
    wheel1 is the *vertical* axis and wheel2 is *horizontal*. The `2` variant
    (fixed six-argument signature) is used because the plain function is
    variadic, which ctypes cannot call safely on Apple Silicon.
    """
    backend, fake_cg, _fake_cf = _mock_backend()

    backend.scroll(3, 7)  # sdx=3, sdy=7

    scroll_calls = [c for c in fake_cg.calls if c[0] == "CGEventCreateScrollWheelEvent2"]
    assert len(scroll_calls) == 1
    _name, args = scroll_calls[0]
    _source, _units, wheel_count, wheel1, wheel2, wheel3 = args
