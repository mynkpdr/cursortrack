...
Real GetCursorPos/SetCursorPos round-trips against user32.dll only run on
Windows. The GetCursorPos-failure regression test builds a WindowsBackend
without running __init__ (which requires loading user32.dll), so it - and
the prototype-declaration checks - run on every platform.
"""
...
requires_windows = pytest.mark.skipif(
    not IS_WINDOWS, reason="Requires a real Win32 user32.dll to round-trip cursor position."
)
