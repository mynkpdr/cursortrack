...
- `get_screen_size()` → `CGMainDisplayID()` + `CGDisplayPixelsWide`/`CGDisplayPixelsHigh`. **Main display only** — the same single-screen scope as the Windows backend's `GetSystemMetrics(SM_CXSCREEN/SM_CYSCREEN)`; multi-monitor parity is tracked in [ROADMAP.md](../ROADMAP.md). `MacOSBackend` does not override `InputBackend.get_screen_bounds()` (used by `play.py`'s fail-safe corner check): its default — origin `(0, 0)`, this method's width/height — is already correct here, since Quartz's global coordinate space is defined with `(0, 0)` at the main display's top-left; there's no separate virtual-desktop-origin query the way Windows needs `SM_XVIRTUALSCREEN`/`SM_YVIRTUALSCREEN`.
